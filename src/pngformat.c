...
CRESULT pngformat_read(PNGFORMAT *v, char* filename) {
	 ...
	 for(int i=0; i < s; i+=4)
        {
            int j = i/4*3;
            v->argb[i+0] = 0xff;
            v->argb[i+1] = v->image_data[j+0];
            v->argb[i+2] = v->image_data[j+1];
    ...
    else if(png_get_color_type(v->png_ptr, v->info_ptr) == PNG_COLOR_TYPE_RGBA && v->bit_depth == 8)
    {
        for(int i=0; i < s; i+=4)
        {
            ...
            v->argb[i+1] = v->image_data[i+0];
            v->argb[i+2] = v->image_data[i+1];
            v->argb[i+3] = v->image_data[i+2];
        }
    }
    else if(png_get_color_type(v->png_ptr, v->info_ptr) == PNG_COLOR_TYPE_GRAY && v->bit_depth == 8)
    {
        for(int i=0; i < s; i+=4)
        {
            int j = i/4;            
            v->argb[i+0] = 0xff;
            v->argb[i+1] = v->image_data[j];
            v->argb[i+2] = v->image_data[j];
            v->argb[i+3] = v->image_data[j];
        }
    }
    else if(png_get_color_type(v->png_ptr, v->info_ptr) == PNG_COLOR_TYPE_GRAY_ALPHA  && v->bit_depth == 8)
    {
        for(int i=0; i < s; i+=4)
        {
            int j = i/4*2;
            v->argb[i+0] = v->image_data[j+1];
            v->argb[i+1] = v->image_data[j];
            v->argb[i+2] = v->image_data[j];
            v->argb[i+3] = v->image_data[j];
        }
